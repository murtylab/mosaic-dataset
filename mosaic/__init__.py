...
def load(
    ...
    assert isinstance(names_and_subjects, dict), f"names_and_subjects must be a dict mapping dataset names (str) to subject_ids (list of integers or 'all'), but got: {type(names_and_subjects)}"

    for dataset_name in names_and_subjects.keys():
        assert dataset_name in name_to_dataset_mapping.keys(), f"Dataset name {dataset_name} is not valid. Please choose from {list(name_to_dataset_mapping.keys())}."
        subject_ids = names_and_subjects[dataset_name]
        assert isinstance(subject_ids, (list, str)), f"For {dataset_name}, subject_ids must be a list of integers or 'all', but got: {type(subject_ids)}"
        if isinstance(subject_ids, list):
            for subject_id in subject_ids:
                assert isinstance(subject_id, int), f"For {dataset_name}, each subject_id must be an integer, but got: {type(subject_id)}"
                assert 1 <= subject_id <= num_subjects[dataset_name], f"For {dataset_name}, subject_id {subject_id} is out of range for dataset {dataset_name}. Must be between 1 and {num_subjects[dataset_name]}."
        else:
            assert subject_ids == "all", f"If subject_ids is a string, it must be 'all', but got: {subject_ids}"
    ...
    for name, subject_id in zip(
        ...
        if subject_id != "all":
            ...
            assert subject_id == "all", f"If subject_id is a string, it must be 'all', but got: {subject_id}"
            dataset = load_single_dataset(
                name=name,
                subject_id=subject_id,
                folder=folder
