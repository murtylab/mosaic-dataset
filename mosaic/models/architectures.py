...
from torchvision.models import alexnet, AlexNet, AlexNet_Weights, resnet50, ResNet, ResNet50_Weights, ResNet18_Weights, resnet18, mobilenet_v2, MobileNet_V2_Weights, squeezenet1_1, SqueezeNet1_1_Weights, swin_t, Swin_T_Weights
...
class CNN(nn.Module):
    def __init__(self, num_outputs=91282, num_subjects=93, subject_embedding_dim=16, angle_embedding_dim=8):
        ...
        self.fc_combined = nn.Linear(256 + subject_embedding_dim + angle_embedding_dim, max(512, num_outputs // 2))
...
class RegressionAlexNet(nn.Module):
    def __init__(self, num_outputs: int=91282, subjects: list=[], subject_embedding_dim: Optional[int]=128, pretrained: bool = False, bottleneck_dim: int = 512, device: str='cpu'):
        """
        ...
        self.subjectID2idx = {subjectID: idx for idx, subjectID in enumerate(self.subjects)}
        self.device = device
        ...
        if self.subject_embedding_dim:
            self.subject_embedding = nn.Embedding(len(self.subjects), subject_embedding_dim)
...
class ResNet50Core(nn.Module):
    def __init__(self, pretrained: bool = False):
        ...
        new_features = [original_resnet50.conv1,
...
class ResNet18Core(nn.Module):
    def __init__(self, pretrained: bool = False):
        ...
        #use all layers except last avgpool and fc
        new_features = [original_resnet18.conv1,
            ...
            for layer in original_model.features:
                if isinstance(layer, nn.Conv2d):
                    ...
                    new_features.append(nn.Sequential(
                        layer,
                        nn.BatchNorm2d(layer.out_channels) if self.add_batchnorm else nn.Identity()
                    ))
...
class Encoder(nn.Module):
    ...
    def count_parameters(self):
        ...
        readout_params = sum(p.numel() for p in self.readout.parameters() if p.requires_grad)
...
class EncoderMultiHead(nn.Module):
    def __init__(self, core, readout_class, subjectID2idx, confidence_scores=None, **readout_kwargs):
        super().__init__()
        self.subjectID2idx = subjectID2idx
        self.idx2subjectID = {idx: subjectID for subjectID, idx in subjectID2idx.items()}
        self.core = core
    ...
    def forward(self, x, subjectID_value, data_key=None, detach_core=False, fake_relu=False, **kwargs):
        ...
        if not detach_core and self.training:
            ...
            confidence_tensor = torch.tensor(batch_confidence_scores, 
            ...
            core_output.register_hook(lambda grad: grad * confidence_tensor.view(-1, 1, 1, 1))
        ...
        for i in range(batch_size):
            ...
            if "sample" in kwargs:
                sample_output = self.subject_readouts[subjectID_str](core_output[i:i+1], sample=kwargs["sample"])
            else:
                sample_output = self.subject_readouts[subjectID_str](core_output[i:i+1])
    ...
    def count_parameters(self):
        ...
        readout_params = sum(
            p.numel() for name, module in self.subject_readouts.items() 
