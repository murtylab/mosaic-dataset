...
def from_pretrained(
    ...
    vertices: Union[str, list] = "all",
    framework: str = "multihead",
    subjects: Union[str, list] = "all",
) -> nn.Module:
    assert backbone_name in valid_backbone_names, \
        f"Invalid backbone_name {backbone_name}. Must be one of {valid_backbone_names}"
    assert vertices in valid_vertices, f"Invalid vertices: {vertices}. Must be one of: {valid_vertices}"
